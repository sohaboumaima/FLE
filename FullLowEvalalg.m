function [xsol, fsol, output] = FullLowEvalalg(fun, x0, alpha0, pr, dirgen, maxevals, tol_feas, unconst)
...
while m  % The problem is already at the solution or infeasible when m = 0
    ...
    switch evl
        case "FE"
            % Full eval step
            [xn, fn, g, H, beta, success, nf] = fullevalstep(fun, x, f, H, x_old, g_old, h, Z, numfe, pr, alpha, unconst);
            if (numIter == 1)
                ng0 = norm(g);
            end
